...
# Example algorithms
class _OnePlusOneES(object):
    """
        Implementation of the default (1+1)-ES
        Requires the length of the vector to be optimized and the budget
    ...
    def __init__(self, n, budget):
        self.n = n
        self.parameters = Parameters(n, budget, mu=1, lambda_=1)
        self.population = [FloatIndividual(n)]

        # We use functions here to 'hide' the additional passing of parameters that are algorithm specific
        self.recombine = Rec.onePlusOne
        self.mutate = partial(Mut.addRandomOffset, sampler=Sam.GaussianSampling(n))
        self.select = Sel.onePlusOneSelection
        self.mutateParameters = self.parameters.oneFifthRule

    def setBudget(self, budget):
        """
            Update the evaluation budget

            :param budget:  Allowed evaluation budget for the optimization run
        """
    ...
    def runOptimizer(self, fitnessFunction):
        ...
        functions = {
            ...
            'mutate': self.mutate,
            'select': self.select,
            'mutateParameters': self.mutateParameters,
        }

        _, results = baseAlgorithm(self.population, fitnessFunction, self.parameters.budget, functions, self.parameters)
...
class _CMA_ES(object):
    ...
    def __init__(self, n, budget, mu=None, lambda_=None, elitist=False):
        self.n = n
        ...
        self.mutate = partial(Mut.CMAMutation, sampler=Sam.GaussianSampling(n))
...
class _OnePlusOneCholeskyCMAES(object):
    """
        Implementation of the default (1+1)-Cholesky CMA-ES
        Requires the length of the vector to be optimized, the handle of a fitness function to use and the budget
    ...
    def __init__(self, n, budget):
        ...
        # We use functions here to 'hide' the additional passing of parameters that are algorithm specific
        self.recombine = Rec.onePlusOne
        self.mutate = partial(Mut.choleskyCMAMutation, sampler=Sam.GaussianSampling(n))

        def select(pop, new_pop, _, params):
            return Sel.onePlusOneCholeskySelection(pop, new_pop, params)
        self.select = select
        self.mutateParameters = self.parameters.adaptCholeskyCovarianceMatrix
...
class _OnePlusOneActiveCMAES(object):
    """
        Implementation of the Active (1+1)-Cholesky CMA-ES
        Requires the length of the vector to be optimized, the handle of a fitness function to use and the budget
    ...
    def __init__(self, n, budget):
        ...
        def select(pop, new_pop, _, params):
            return Sel.onePlusOneCholeskySelection(pop, new_pop, params)
        self.select = select
        self.mutateParameters = self.parameters.adaptActiveCovarianceMatrix
...
class _CMSA_ES(object):
    ...
    def __init__(self, n, budget, mu=None, lambda_=None, elitist=False):
        self.n = n
        self.parameters = Parameters(n, budget, mu, lambda_, elitist=elitist, weights_option='1/n')
        self.population = [FloatIndividual(n) for _ in range(mu)]
...
# The eventual Big Daddy of them all: BaseOptimizer
...
:returns:               The statistics generated by running the algorithm
    ...
    def runOptimizer(self):
        """
        Perform the overall optimization. Iterate over the atomic EA-steps while the given budget lasts.
        Periodically checks if any of the restart-criteria have been violated and if a restart is required.

        :return:
        """
        pass

    def checkRestartCriteria(self):
        """
        Check the state of the optimizer: has it stagnated? Diverged? Degenerated?
    ...
    def initialize(self):
        """
        Perform the specific setups required before actual optimization can occur. These are the same steps that are
        repeated for a reset on a local restart of the optmizer.
    ...
    def runIterate(self):
        """
        Perform a single iteration of the optimization by calling the atomic EA-steps in order.
        :return:
        """
        pass
...
:returns:               The statistics generated by running the algorithm
    ...
    def __init__(self, population, fitnessFunction, budget, functions, parameters, parallel=False, debug=False):
    ...
    def runOptimizer(self):
        ...
        seq_cutoff = self.parameters.mu_int * self.parameters.seq_cutoff

        # Initialization
        used_budget = 0
        recombine = self.functions['recombine']
        mutate = self.functions['mutate']
        select = self.functions['select']
        mutateParameters = self.functions['mutateParameters']
        ...
        new_population = recombine(self.population, self.parameters)
        mutEval = partial(_mutateAndEvaluate, mutate=mutate, fitFunc=self.fitnessFunction)
        ...
        while used_budget < self.budget:
            ...
            if self.parallel:
                if Config.use_MPI:
                    ...
                    for i in range(int(ceil(len(new_population) / Config.GA_num_parallel))):
                        ...
                        for ind in new_population[i * Config.GA_num_parallel:(i + 1) * Config.GA_num_parallel]:
                            mutate(ind, self.parameters)
                            genes.append(ind.genotype)

                        fitnesses.extend(self.fitnessFunction(genes))
                ...
                if sequential_evaluation:
                    for i, individual in enumerate(new_population):
                        ...
                        if i >= self.parameters.seq_cutoff and improvement_found:
                            improvement_found = False  # Have we evaluated at least mu mutated individuals?
                            break
                        if used_budget == self.budget:
                            break
                else:
                    used_budget += self.parameters.lambda_
                    i = self.parameters.lambda_
            else:  # Sequential
                for i, individual in enumerate(new_population):
                    mutate(individual, self.parameters)  # Mutation
                    # Evaluation
            ...
            self.population = select(self.population, new_population, used_budget, self.parameters)  # Selection

            # Track parameters
            gen_size = used_budget - len(fitness_over_time)
            generation_size.append(gen_size)
            sigma_over_time.extend([self.parameters.sigma_mean] * gen_size)
            fitness_over_time.extend([self.population[0].fitness] * gen_size)
            if self.population[0].fitness < best_individual.fitness:
                best_individual = copy(self.population[0])
            ...
            if len(self.population) == self.parameters.mu_int:
                new_population = recombine(self.population, self.parameters)  # Recombination
            else:
                print('Error encountered in baseAlgorithm():\n'
                      'Bad population size! Size: {} instead of {} at used budget {}'.format(len(self.population),
            ...
            if two_point_adaptation:
                ...
                used_budget += 2
                if used_budget > self.budget and sequential_evaluation:
            ...
            # Local restart
            if self.parameters.localRestart(used_budget, fitnesses):
                break

        # Debug print statement, displaying the number of degenerations that occurred during the run of this algorithm
        if self.parameters.count_degenerations and self.debug:
            print(self.parameters.count_degenerations, end=' ')
