...
def localRestartAlgorithm(fitnessFunction, budget, functions, parameter_opts, parallel=False):
    ...
    lambda_ = {'small': None, 'large': lambda_init}
    budgets = {'small': None, 'large': None}
    regime = 'first'  # Later alternates between 'large' and 'small'
    ...
    while local_budget > 0:
        ...
        if parameter_opts['local_restart'] == 'IPOP':
            ...
            try:
                budgets[regime] -= used_budget
                regime = 'small' if budgets['small'] > budgets['large'] > 0 else 'large'
            except IndexError:  # Setup of the two regimes after running regularily for the first time
                budgets['small'] = local_budget // 2
                budgets['large'] = local_budget - budgets['small']
                regime = 'large'
            ...
            if regime == 'large':
                ...
                lambda_['small'] = int(floor(lambda_init * (.5 * lambda_['large']/lambda_init)**rand_val))
                parameter_opts['sigma'] = 2e-2*np.random.random()

            parameter_opts['lambda'] = lambda_[regime]
